...
def display_serial(ser, args):
    ...
    try:
        while True:
            ...
            print(img.shape)
            axs.imshow(img, cmap="jet", vmax=32, vmin=25, extent=(0, img.shape[1], img.shape[0], 0))
            axs.set_title(img_misc)
            axs.axvline(20, color="red")
            for bbox in bboxes:
            ...
            for centroid in centroids:
                ...
                if len(centroid) > 3:
                    ...
                    if np.sqrt(dist_squared) < 1:
                        continue
                    arrow_patch = patches.Arrow(
                        x=x_prev, y=y_prev, dx=x - x_prev, dy=y - y_prev, edgecolor="black", facecolor="white", width=4
                    )
                    axs.add_patch(arrow_patch)
